...
FORBIDDEN_FIELD_NAMES = ["id", "tbl"]

class FieldDefn
  attr_accessor :name, :column_name, :data_type, :args, :ordinal, :default_value, :association
  def initialize(name, data_type, ordinal, args = {})
    @name = FORBIDDEN_FIELD_NAMES.include?(name) ? "_#{name}" : name
    @column_name = name
    @data_type = data_type
    @args = args
    @ordinal = ordinal
    ...
    if !@default_value.nil?
      if data_type == :datetime || data_type == :date
        @default_value = (Time.at(@default_value.to_i).to_i * 1000).to_s.concat("L")
      end
    end
  ...
  def get_from_rs
    case data_type
      when :integer
        is_long? ? "getLongOrNull(rs, \"#{column_name}\")" : "getIntOrNull(rs, \"#{column_name}\")"
      when :bigint
        "getLongOrNull(rs, \"#{column_name}\")"
      when :datetime
        "getDateAsLong(rs, \"#{column_name}\")"
      when :date
        "getDateAsLong(rs, \"#{column_name}\")"
      when :float
        "getDoubleOrNull(rs, \"#{column_name}\")"
      when :decimal
        "getDoubleOrNull(rs, \"#{column_name}\")"
      when :boolean
        "getBooleanOrNull(rs, \"#{column_name}\")"
      else
        "rs.get#{prep_stmt_type}(\"#{column_name}\")"
    end
  end
